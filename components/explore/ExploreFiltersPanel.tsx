...
export function ExploreFiltersPanel({
  ...
  return (
    <div
      id={id}
      className={cn(
        "flex flex-col h-full max-h-[calc(100vh-4rem)] lg:max-h-[calc(100dvh-6rem)] overflow-hidden border border-border/70 bg-bg-card/30 shadow-sm backdrop-blur-md",
        className
      )}
    >
      {/* Header - Improved to be more "header-like" */}
      <div className="sticky flex justify-between top-0 z-20 border-b border-border/70 backdrop-blur-md px-5 py-4 shrink-0">
        <div className="flex flex-wrap items-center gap-2">
          {activeChips.length ? (
            activeChips.map((chip) => (
              <Pill
                key={chip.key}
                className="bg-bg-input text-text-secondary dark:bg-secondary-900/40 border border-border/60 shadow-xs"
              >
                {chip.label}
              </Pill>
            ))
      ...
      <div className="flex-1 px-5 py-5 overflow-y-auto overscroll-contain scrollbar-hide">
        <div className="space-y-6">
          ...
          />
          {/* Spacer for bottom sticky footer */}
          <div className="h-4" />
        </div>
      </div>

      {/* Footer - Sticky at bottom without offset */}
      <div className="sticky bottom-0 z-20 border-t border-border bg-bg-card/95 backdrop-blur-md px-5 py-4 shrink-0 pb-[max(env(safe-area-inset-bottom),1rem)] rounded-b-none">
