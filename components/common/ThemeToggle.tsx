...
export function ThemeToggle() {
  ...
  if (!mounted) {
    return (
      <div className="h-9 w-18 sm:h-11 sm:w-22 rounded-full border border-border)] bg-(--surface) p-1 opacity-0" />
    );
  }
  ...
  return (
    <button
      ...
      className="group relative flex h-9 w-18 sm:h-11 sm:w-22 items-center rounded-full border border-border bg-(--surface) p-1 shadow-[0_0_20px_rgba(46,86,233,0.15)] transition-all duration-300 hover:border-(--accent)/50 hover:shadow-[0_0_28px_rgba(46,86,233,0.25)]"
      type="button"
    >
      <motion.div
        layout
        ...
        className="absolute h-7 w-8 sm:h-9 sm:w-10 rounded-full bg-linear-to-br from-[#1A9CFF] via-[#2E56E9] to-[#5B21B6] shadow-lg"
      >
          ...
          <SunIcon
            className={`h-4 w-4 sm:h-4.5 sm:w-4.5 transition-colors duration-300 ${
              theme === "light"
                ...
                : "text-text-secondary group-hover:text-yellow-200"
            }`}
          />
        </motion.div>
        ...
        >
          <MoonStar
            className={`h-4 w-4 sm:h-4.5 sm:w-[4.5 transition-colors duration-300 ${
              theme === "dark"
                ? "text-white drop-shadow-[0_0_4px_rgba(255,255,255,0.5)]"
                : "text-text-secondary group-hover:text-slate-600"
            }`}
          />
        </motion.div>
      </div>
