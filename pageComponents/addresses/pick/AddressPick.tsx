...
import { SearchParamsProps } from "@/app/(pages)/addresses/pick/page";
...
export default function AddressPickpage({searchParams}: Props) {
  const { t } = useTranslation();
  const { showToast } = useToast();
  const router = useRouter();

  const addressId = (searchParams.addressId ?? "").trim() || null;
